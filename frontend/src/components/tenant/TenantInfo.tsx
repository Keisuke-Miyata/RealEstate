...
const TenantInfo: FC<SeekerDetailsProps> = ({ post }) => {
  const dateOnly: string = new Date(post.preferredMoveDate).toISOString().split('T')[0];
  const today: string = new Date().toISOString().split('T')[0];
  const moveDate: string = dateOnly < today ? 'Now' : dateOnly;

  const hasPartner: boolean = !!(post.partnerName || post.partnerGender || post.partnerNationality || post.partnerFieldOfStudy);

  const hasGroupMembers: boolean = !!(post.groupMembers && post.groupMembers.length > 1);
