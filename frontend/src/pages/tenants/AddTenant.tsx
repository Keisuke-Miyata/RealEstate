...
import { TenantFormData, GroupMember } from "../../shared/types";
...
const AddTenant: FC = () => {
    ...
    const [groupMembers, setGroupMembers] = useState<GroupMember[]>([{ id: Date.now() }]);
    ...
    const { data: tenant, isLoading: tenantLoading } = useQuery(
        ...
        () => getTenant(id as string),
        {
            ...
            onSuccess: (data) => {
                ...
                setFormData({
                    ...data,
                    preferredMoveDate: data.preferredMoveDate && typeof data.dateAvailability === "string"
                        ? new Date(data.preferredMoveDate)
                        : new Date(),
                });
            }
        }
    ...
    const addGroupMember = (): void => {
        setGroupMembers([...groupMembers, { id: Number(Date.now()) }]);
    };

    // Remove a specific group member
    const removeGroupMember = (id: number): void => {
    ...
    //             })),
    //     }));
    //     console.log("this is groupMembers", formData.groupMembers)
    // };
    ...
    //     try {
    //         if (id) {
    //             await updateTenant(id, formData, userDetails.token ?? "");
    //             toast.success("Tenant updated successfully", { position: "bottom-right" });
    //         }
    //         else {
    //             await createTenant(formData, userDetails.token ?? "", user?.email ?? "");
...
const handleSubmit = async (e: any): Promise<void> => {
    ...
    updateFormData(); // This triggers the update
};

// **Wait for `formData` to update before continuing**
useEffect(() => {
    if (!isSubmitting) return; // Only proceed if submission was triggered
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="form-container">
                    <NumberInput
                        ...
                        onChange={(value) => setFormData({ ...formData, monthlyBudget: value ? Number(value) : 0 })}
                    />
                ...
                <DateInput
                    ...
                    className="mb-10"
                    value={formData?.preferredMoveDate || new Date()}
                    onChange={(value) => setFormData((prev) => ({ ...prev, preferredMoveDate: value ? new Date(value) : new Date() }))}
                />
                <div className="form-container">
                    <Select
