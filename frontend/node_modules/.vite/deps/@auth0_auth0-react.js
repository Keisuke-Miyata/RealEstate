...
var E = (e2) => {
  ...
  if (e2.organizationId) {
    ...
    if (e2.organizationId !== t2.claims.org_id) throw new Error(`Organization ID (org_id) claim mismatch in the ID token; expected "${e2.organizationId}", found "${t2.claims.org_id}"`);
  }
  return t2;
};
...
var te = class {
  ...
  async loginWithPopup(e2, t2) {
    ...
    const n2 = await ((e3) => new Promise((t3, i3) => {
      ...
      o3 = function(s2) {
        if (s2.data && "authorization_response" === s2.data.type) {
          if (clearTimeout(a3), clearInterval(n3), window.removeEventListener("message", o3, false), e3.popup.close(), s2.data.response.error) return i3(d.fromPayload(s2.data.response));
          t3(s2.data.response);
        }
    ...
    if (o2.state !== n2.state) throw new Error("Invalid state");
  ...
  async handleRedirectCallback(e2 = window.location.href) {
    ...
    if (!s2) throw new Error("Invalid state");
    if (this.transactionManager.remove(), n2) throw new u(n2, a2 || n2, i2, s2.appState);
    if (!s2.code_verifier || s2.state && s2.state !== i2) throw new Error("Invalid state");
  ...
  async _getTokenFromIFrame(e2) {
    ...
    try {
      ...
      if (n2 !== i3.state) throw new Error("Invalid state");
